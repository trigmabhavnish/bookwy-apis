...
feedBackSchema.getFeedBackDetails = function (obj, result) {
    // let userId = obj.userId
    sql("SELECT fw_feedback.project_id,fw_feedback.user_id,fw_feedback.status,fw_feedback.feed_desc,fw_feedback.feed_con,fw_feedback.feed_date,fw_feedback.overall_rate,fw_user.user_id,fw_user.user_name,fw_project.id,fw_project.project_name FROM fw_feedback INNER JOIN fw_user ON fw_feedback.user_id=fw_user.user_id INNER JOIN fw_project ON fw_feedback.project_id=fw_project.id where fw_feedback.user_id = ? AND fw_feedback.project_id = ?", [obj.userId, obj.projectId], function (err, feedback) {
