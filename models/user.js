...
userSchema.fetchUserByAuthToken = function (authToken, result) {
    sql("Select user_id , account_balance, director_id ,profile_pic, first_name , last_name, user_name  from fw_user where auth_token = ? and status = 'Y'", authToken, function (err, res) {    
        if (err) {
            //console.log(err);              
            result(err, null);
        } else {
